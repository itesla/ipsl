...
model CSVGN1 "STATC SHUNT COMPENSATOR MODEL"
  ...
  Modelica.Blocks.Interfaces.RealInput V annotation(Placement(transformation(extent = {{-118, -18}, {-92, 8}}), iconTransformation(extent = {{-13, -13}, {13, 13}}, rotation = 180, origin = {39, 29})));
  ...
  iPSL.NonElectrical.Continuous.SimpleLagLim ImSimpleLagnonwinduplimit(outMin = VMIN, outMax = VMAX, T = T3, K = K, y_start = k30) annotation(Placement(transformation(extent = {{-42, -12}, {-22, 8}})));
  ...
  iPSL.Connectors.PwPin p(vr(start = vr0), vi(start = vi0), ir(start = ir0), ii(start = ii0)) annotation(Placement(transformation(extent = {{108, 6}, {128, 26}}), iconTransformation(extent = {{-10, 42}, {10, 62}})));
protected
...
equation
  ...
  connect(const1.y, add.u1) annotation(Line(points = {{44.5, 59}, {61, 59}, {61, 18}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(add.y, Y) annotation(Line(points = {{72.5, 15}, {95, 15}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(product1.y, add.u2) annotation(Line(points = {{44.4, 12}, {61, 12}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(V, add1.u2) annotation(Line(points = {{-105, -5}, {-93.5, -5}, {-93.5, -5.2}, {-77.4, -5.2}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(const2.y, add1.u1) annotation(Line(points = {{-97.3, 21}, {-82, 21}, {-82, 3.2}, {-77.4, 3.2}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(const.y, product1.u1) annotation(Line(points = {{16.5, 59}, {24, 59}, {24, 14.4}, {35.2, 14.4}}, color = {0, 0, 127}, smooth = Smooth.None));
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics = {Text(extent=  {{-144, 92}, {-108, 84}}, lineColor=  {0, 0, 255}, textString=  "Simplified one")}), Icon(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}), graphics={  Rectangle(extent=  {{-60, 42}, {52, -68}}, lineColor=  {0, 0, 255}), Text(extent=  {{-46, 20}, {40, -26}}, lineColor=  {0, 0, 255}, textString=  "CSVGN1")}), Documentation(info = "<html>
