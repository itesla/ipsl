...
model WT3G1
  ...
  Modelica.Blocks.Continuous.LimIntegrator imIntegrator1(
    ...
    y_start=0,
    initType=Modelica.Blocks.Types.Init.InitialOutput)
    annotation (Placement(transformation(extent={{-32,-10},{-22,0}})));
  ...
  Modelica.Blocks.Math.Gain imGain(k=-1/Xeq)
    annotation (Placement(transformation(extent={{2,64},{20,82}})));
  Modelica.Blocks.Interfaces.RealOutput Iy(start=Iy0) annotation (Placement(
        transformation(extent={{50,26},{68,44}}), iconTransformation(extent={{
            50,26},{68,44}})));
  Modelica.Blocks.Interfaces.RealOutput Ix(start=Ix0) annotation (Placement(
        transformation(extent={{50,2},{68,20}}), iconTransformation(extent={{50,
            2},{68,20}})));
  Modelica.Blocks.Interfaces.RealOutput Iterm(start=Iy0) annotation (Placement(
        transformation(extent={{108,24},{126,42}}), iconTransformation(extent={
            {108,24},{126,42}})));
  Modelica.Blocks.Interfaces.RealInput Eqcmd(start=Eqcmd0) annotation (
      Placement(transformation(extent={{-120,54},{-82,92}}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={98,84})));
...
equation
  ...
  connect(Eqcmd, imSimpleLag.u) annotation (Line(points={{-101,73},{-66.5,73},{
          -66.5,73},{-31.8,73}}, color={0,0,127}));
  connect(Ipcmd, imSimpleLag1.u) annotation (Line(points={{-101,39},{-66.5,39},
          {-66.5,39},{-31.8,39}}, color={0,0,127}));
  connect(add.y, imLimited.u) annotation (Line(points={{10.9,-15},{12.45,-15},{
          15,-15}}, color={0,0,127}));
  connect(imLimited.y, imIntegrator.u) annotation (Line(points={{26.5,-15},{
          27.65,-15},{33,-15}}, color={0,0,127}));
  connect(imIntegrator.y, delta) annotation (Line(points={{44.5,-15},{48.25,-15},
          {59,-15}}, color={0,0,127}));
  connect(imGain1.y, add.u2) annotation (Line(points={{-39.4,-20},{-9.8,-20},{-9.8,
          -20.4}}, color={0,0,127}));
  connect(imIntegrator1.y, add.u1) annotation (Line(points={{-21.5,-5},{-14,-5},
          {-14,-9.6},{-9.8,-9.6}}, color={0,0,127}));
  connect(imIntegrator1.u, add.u2) annotation (Line(points={{-33,-5},{-36,-5},{
          -36,-20},{-9.8,-20},{-9.8,-20.4}}, color={0,0,127}));
  connect(Vy, imGain1.u) annotation (Line(points={{-99,-21},{-76.5,-21},{-76.5,
          -20},{-53.2,-20}}, color={0,0,127}));
  connect(imSimpleLag.y, imGain.u) annotation (Line(points={{-11.1,73},{-4.55,
          73},{-4.55,73},{0.2,73}}, color={0,0,127}));
  connect(imGain.y, Iy) annotation (Line(points={{20.9,73},{40,73},{40,35},{59,
          35}}, color={0,0,127}));
  connect(imSimpleLag1.y, Ix) annotation (Line(points={{-11.1,39},{22,39},{22,
          11},{59,11}}, color={0,0,127}));
  annotation (
    Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{
            100,100}})),
    Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{100,
            100}}), graphics={Rectangle(extent={{-102,96},{108,-48}}, lineColor
          ={0,0,255}),Text(
          extent={{-40,76},{44,38}},
          lineColor={0,0,255},
          textString="WT3G1")}),
