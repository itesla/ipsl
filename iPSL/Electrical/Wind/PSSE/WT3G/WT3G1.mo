...
model WT3G1
  ...
  iPSL.Connectors.PwPin p(
    ...
    vi(start=vi0),
    ir(start=ir1),
    ii(start=ii1)) annotation (Placement(transformation(extent={{-100,34},{-80,54}}), iconTransformation(extent={{-100,34},{-80,54}})));
  Modelica.Blocks.Continuous.Integrator imIntegrator(
    k=wbase,
    y_start=anglev_rad,
    initType=Modelica.Blocks.Types.Init.InitialOutput) annotation (Placement(transformation(extent={{34,-20},{44,-10}})));
  Modelica.Blocks.Continuous.LimIntegrator imIntegrator1(
    outMin=-Pllmax,
    outMax=Pllmax,
    k=0,
    y_start=0,
    initType=Modelica.Blocks.Types.Init.InitialOutput) annotation (Placement(transformation(extent={{-32,-10},{-22,0}})));
  ...
  Modelica.Blocks.Interfaces.RealInput Eqcmd(start=Eqcmd0)
    annotation (Placement(transformation(extent={{-120,54},{-82,92}}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={98,84})));
  Modelica.Blocks.Interfaces.RealInput Ipcmd(start=Ipcmd0)
    annotation (Placement(transformation(extent={{-120,20},{-82,58}}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={98,56})));
  ...
  Modelica.Blocks.Interfaces.RealOutput ipcmd0=Ipcmd0
    annotation (Placement(transformation(extent={{30,-86},{48,-68}}), iconTransformation(
        extent={{-9,-9},{9,9}},
        rotation=90,
        origin={13,105})));
  Modelica.Blocks.Interfaces.RealOutput eqcmd0=Eqcmd0
    annotation (Placement(transformation(extent={{30,-62},{48,-44}}), iconTransformation(
        extent={{-9,-9},{9,9}},
        rotation=90,
        origin={-27,105})));
protected
...
equation
  ...
  annotation (
    ...
    Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{100,100}}), graphics={Rectangle(extent={{-102,96},{108,-48}}, lineColor={0,0,255}),Text(
          extent={{-40,76},{44,38}},
          lineColor={0,0,255},
          textString="WT3G1")}),
