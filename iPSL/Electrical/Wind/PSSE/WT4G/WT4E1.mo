...
model WT4E1 "Electrical Control for Type 4 Wind Generator"
  ...
  Modelica.Blocks.Interfaces.RealInput P(start = p0) annotation(Placement(transformation(extent = {{-210, -10}, {-190, 10}}), iconTransformation(extent = {{-20, -20}, {20, 20}}, rotation = 180, origin = {180, 40})));
  Modelica.Blocks.Interfaces.RealInput Q(start = q0) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 0, origin = {-200, 188}), iconTransformation(extent = {{-20, -20}, {20, 20}}, rotation = 180, origin = {180, 100})));
  Modelica.Blocks.Interfaces.RealInput V(start = v0) annotation(Placement(transformation(extent = {{-210, -170}, {-190, -150}}), iconTransformation(extent = {{-20, -20}, {20, 20}}, rotation = 180, origin = {180, 160})));
  ...
  Modelica.Blocks.Continuous.LimIntegrator K6(outMin = VMINCL, outMax = VMAXCL, k = KQI, y_start = k60) annotation(Placement(transformation(extent = {{80, 50}, {100, 70}})));
  ...
  iPSL.Electrical.Wind.PSSE.Submodels.CCL cCL(Qmax = QMX, ImaxTD = ImaxTD, Iphl = Iphl, Iqhl = Iqhl, pqflag = PQFLAG) annotation(Placement(transformation(extent = {{150, -30}, {190, 10}})));
  Modelica.Blocks.Sources.Constant Qcmd0(k = Qref) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 0, origin = {-100, 0})));
  ActivePowerController activePowerController(Kpp = Kpp, KIP = KIP, Kf = Kf, Tf = Tf, dPMX = dPMX, dPMN = dPMN, T_Power = T_Power, Pref = Pref, k20 = k20, k30 = k30, k50 = k50, p0 = p0) annotation(Placement(transformation(rotation = 0, extent = {{40, -100}, {80, -60}})));
  pf_Controller PF_Controller(Tp = Tp, PFA_ref = PFA_ref, p0 = p0) annotation(Placement(transformation(rotation = 0, extent = {{-120, 28}, {-80, 68}})));
  windControlEmulator windControlEmulator1(Tfv = Tfv, Kpv = Kpv, KIV = KIV, QMX = QMX, QMN = QMN, Tv = Tv, Vref = Vref, k0 = k0, k10 = k10, k80 = k80) annotation(Placement(transformation(rotation = 0, extent = {{-120, 140}, {-80, 180}})));
...
initial equation
  ...
  k90 = p0;
equation
  connect(Q, feedback1.u2) annotation(Line(points = {{-200, 188}, {60, 188}, {60, 68}}, color = {0, 0, 150}, smooth = Smooth.None));
  connect(V, Vcl.u2) annotation(Line(points = {{-200, -160}, {118, -160}, {118, 52}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(cCL.Vt, V) annotation(Line(points = {{150, -10}, {92, -10}, {92, -160}, {-200, -160}}, color = {255, 0, 0}, smooth = Smooth.None));
  connect(Qord.y, feedback1.u1) annotation(Line(points = {{41, 60}, {52, 60}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(K7.y, WIQCMD) annotation(Line(points = {{163, 60}, {210, 60}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(K7.u, Vcl.y) annotation(Line(points = {{140, 60}, {127, 60}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(K6.u, feedback1.y) annotation(Line(points = {{78, 60}, {69, 60}}, color = {0, 0, 127}, smooth = Smooth.None));
...
equation
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-200, -200}, {200, 200}}, initialScale = 0.05), graphics = {Rectangle(extent = {{46, 118}, {192, 24}}, lineColor = {255, 0, 0}, lineThickness = 0.5, pattern = LinePattern.Dash), Text(extent = {{64, 116}, {170, 108}}, lineColor = {255, 0, 0}, pattern = LinePattern.Dash, lineThickness = 0.5, fillColor = {0, 0, 255}, fillPattern = FillPattern.Solid, textString = "Reactive Power Control"), Rectangle(extent = {{-134, 130}, {14, -20}}, lineColor = {0, 140, 72}, lineThickness = 0.5, pattern = LinePattern.Dash), Text(extent = {{-130, 128}, {-24, 120}}, lineColor = {0, 140, 72}, pattern = LinePattern.Dash, lineThickness = 0.5, fillColor = {0, 0, 255}, fillPattern = FillPattern.Solid, textString = "Reactive Power Reference Switching"), Rectangle(extent = {{104, 104}, {190, 26}}, lineColor = {102, 44, 145}, lineThickness = 0.5, pattern = LinePattern.Dash), Text(extent = {{92, 104}, {198, 96}}, lineColor = {180, 56, 148}, pattern = LinePattern.Dash, lineThickness = 0.5, fillColor = {0, 0, 255}, fillPattern = FillPattern.Solid, textString = "Terminal Voltage Control")}), Icon(coordinateSystem(extent = {{-200, -200}, {200, 200}}, preserveAspectRatio = false, initialScale = 0.05), graphics = {Rectangle(extent = {{-200, 200}, {200, -200}}, lineColor = {28, 108, 200}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid), Text(extent = {{-70, 42}, {82, -36}}, lineColor = {28, 108, 200}, textString = "WT4E1"), Text(extent = {{110, 172}, {170, 142}}, lineColor = {28, 108, 200}, lineThickness = 0.5, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "V"), Text(extent = {{110, 114}, {170, 84}}, lineColor = {28, 108, 200}, lineThickness = 0.5, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "Q"), Text(extent = {{110, 56}, {170, 26}}, lineColor = {28, 108, 200}, lineThickness = 0.5, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "P"), Text(extent = {{82, -42}, {194, -74}}, lineColor = {28, 108, 200}, lineThickness = 0.5, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "WIPCMD"), Text(extent = {{82, -124}, {194, -156}}, lineColor = {28, 108, 200}, lineThickness = 0.5, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, textString = "WIQCMD")}), Documentation(revisions = "<html>
<!--DISCLAIMER-->
