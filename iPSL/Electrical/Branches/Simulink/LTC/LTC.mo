...
model LTC
  ...
  Modelica.Blocks.Continuous.LimIntegrator            imLimitedIntegrator(outMax = 1, k = 1, outMin = -delay1, y_start = -delay1,
    initType=Modelica.Blocks.Types.Init.InitialOutput)                                                                            annotation(Placement(transformation(extent={{62,8},{
            78,24}})));
  ...
  Modelica.Blocks.Continuous.LimIntegrator  imLimitedIntegrator1(outMin = Ymin, outMax = Ymax, k = (Ymax - Ymin) / (positionNo * delay2), y_start = r0,
    initType=Modelica.Blocks.Types.Init.InitialOutput)                                                                                                  annotation(Placement(transformation(extent={{120,8},
            {136,24}})));
  LTC_quantizer lTC_quantizer annotation(Placement(transformation(extent = {{150, 6}, {170, 26}})));
  Modelica.Blocks.Interfaces.RealInput u annotation(Placement(transformation(extent={{-84,12},
            {-64,32}}),                                                                                       iconTransformation(extent = {{-116, 4}, {-80, 42}})));
