...
model PwMachineM10 "Induction Machine. The model corresponds to Eurostag's full model for M10 machine.
                  2014/03/10"
  // Connectors
  PowerSystems.Connectors.ImPin omegaRef(start = omegaRef0) annotation(Placement(transformation(extent = {{-61, -10}, {-41, 10}})));
  PowerSystems.Connectors.PwPin sortie(vr(start = ur0), vi(start = ui0), ir(start = ir0), ii(start = ii0)) annotation(Placement(transformation(extent = {{40, -10}, {60, 10}})));
  //
