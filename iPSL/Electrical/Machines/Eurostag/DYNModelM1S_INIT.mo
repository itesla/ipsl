...
model DYNModelM1S_INIT
  PowerSystems.Connectors.ImPin pin_CM;
  PowerSystems.Connectors.ImPin pin_EFD;
  PowerSystems.Connectors.ImPin pin_LAMBDAF;
  PowerSystems.Connectors.ImPin pin_LAMBDAD;
  PowerSystems.Connectors.ImPin pin_LAMBDAQ1;
  PowerSystems.Connectors.ImPin pin_LAMBDAQ2;
  PowerSystems.Connectors.ImPin pin_LAMBDAAD;
  PowerSystems.Connectors.ImPin pin_LAMBDAAQ;
  PowerSystems.Connectors.ImPin pin_OMEGA;
  PowerSystems.Connectors.ImPin pin_TETA;
  PowerSystems.Connectors.ImPin pin_IQ;
  PowerSystems.Connectors.ImPin pin_ID;
  PowerSystems.Connectors.ImPin pin_UR;
  PowerSystems.Connectors.ImPin pin_UI;
  PowerSystems.Connectors.ImPin pin_TerminalVoltage;
  PowerSystems.Connectors.ImPin pin_FieldCurrent;
  PowerSystems.Connectors.ImPin pin_ActivePowerPNALT;
  PowerSystems.Connectors.ImPin pin_ActivePowerPN;
  PowerSystems.Connectors.ImPin pin_ActivePowerSNREF;
  PowerSystems.Connectors.ImPin pin_ReactivePowerPNALT;
  PowerSystems.Connectors.ImPin pin_ReactivePowerPN;
  PowerSystems.Connectors.ImPin pin_ReactivePowerSNREF;
  PowerSystems.Connectors.ImPin pin_Current;
  ...
  // ------------------------------------------------------
  parameter Real ri = if rTfoIn > 0.0 or xTfoIn > 0.0 then uNResTfo / uNomNw / uNMacTfo / uBMac else 1.0;
  parameter Real rs = if rTfoIn > 0.0 or xTfoIn > 0.0 then (uNResTfo / uNomNw) ^ 2 * SNREF / sNTfo else 1.0;
...
equation
  ...
  // YNN2
  mDSNom = mD0Nom / (1.0 + mDSat * ENom2 ^ (nDSat / 2.0));
  // YXADS
  mQSNom = mQ0Nom / (1.0 + mQSat * ENom2 ^ (nQSat / 2.0));
  // YXAQS
  ...
  elseif IWLMDV == 1 then
    ...
    mDVPu = -1.0 / (ifNom * yScaleNom);
  end if;
  // FMUDMA (calcule dans puingc.f)
  // per unitage de parametres (park2.f)
  // -----------------------------------
  ONE * yScale = if rTfoIn > 0.0 or xTfoIn > 0.0 then SNREF / SN * rrTfo * rrTfo else SNREF / SN;
  // YSCALE
  ...
  // YNN2
  ONE * E0 = sqrt(E02);
  ONE * mDS0 = mD0 / (1.0 + mDSat * E02 ^ (nDSat / 2));
  // YXADS
  ONE * mQS0 = mQ0 / (1.0 + mQSat * E02 ^ (nQSat / 2));
  // YXAQS
