...
model DYNModelM1S_INIT "Initialization model for synchronous mahine M1S.
  ...
  Modelica.Blocks.Interfaces.RealInput pin_CM;
          ...
          parameter Real SN(fixed=false);  //  puissance apparente nominale (SNOMG)
          ...
          parameter Real nDSat;//(fixed = false);
          parameter Real nQSat;//(fixed = false);
          parameter Real mDSatIn;//(fixed = false);
          parameter Real mQSatIn;//(fixed = false);
          parameter Real rStatIn(fixed = false);
          parameter Real lStatIn(fixed = false);
          parameter Real mQ0Pu(fixed=false);
          parameter Real mD0Pu(fixed=false);
          //parameter Real pNomAlt(fixed = false);
          //parameter Real pNom(fixed = false);
          parameter Real lDPu(fixed=false);
          parameter Real rRotIn(fixed=false);
          parameter Real lRotIn(fixed=false);
          ...
          parameter Real mCanPu(fixed=false);
          ...
          // paramètres calcules
          parameter Real yScaleNom = SNREF/SN; // YSCALE
          parameter Real rStatNom = rStatIn * yScaleNom; // YI = RESARM * YSCALE;
          parameter Real lStatNom = lStatIn * yScaleNom; // YLAMDI = READST * YSCALE
          parameter Real lQINom = lStatIn * yScaleNom;   // YLQI = REAQST avec REAQST=READST (puing1.f)
          parameter Real mQ0Nom = mQ0Pu*yScaleNom;        // YMQI = FMUQST(INB2) * YSCALE (YMQINS)
          parameter Real mD0Nom = mD0Pu*yScaleNom;        // YMDI = FMUDST(INB2) * YSCALE (YMDINS)
          parameter Real urNom = 1.0;    // YE
          parameter Real uiNom = 0.0;    // YF
          parameter Real qPuWLMDV = (SN^2 - pPuWLMDV^2)^(1/2);  // QUNIT (pucalif.f)
          parameter Real pNomWLMDV = pPuWLMDV / SNREF;                      // YPG (pucalif.f)
          parameter Real qNomWLMDV = qPuWLMDV / SNREF;                   // YQG (pucalif.f)
          parameter Real irNom = (pNomWLMDV * urNom + qNomWLMDV * uiNom); // YIR (pucalif.f)
          parameter Real iiNom = (pNomWLMDV * uiNom - qNomWLMDV * urNom); // YII
          parameter Real omegaNom = 1.0; // OMEGA
          parameter Real rTfoNom = 0.0;  // YTI
          parameter Real xTfoNom = 0.0;  // YXTI
            // per unitage complementaire, calcul de rrTfo (puingc.f)
          // ------------------------------------------------------
          parameter Real ri = if
                                ( rTfoIn > 0. or xTfoIn > 0.)  then (uNResTfo/uNomNw) / (uNMacTfo/uBMac) else 1.;
          parameter Real rs = if
                                ( rTfoIn > 0. or xTfoIn > 0.)  then  (uNResTfo/uNomNw)^2 * SNREF / sNTfo else 1.;
          ...
          Real iQNom(start=irNom); // YIQ
          Real uDNom(start=-uiNom); // YUD
          Real uQNom(start=urNom); // YUQ
          Real lambdaADNom(start=lambdaADNom0); // YLAMAD
          Real lambdaAQNom(start=lambdaAQNom0); // YLAMAQ
          Real lambdaADNom2(start=lambdaADNom0*lambdaADNom0); // YV72
          Real lambdaAQNom2(start=lambdaAQNom0*lambdaAQNom0); // YV82
          Real ENom2(start= lambdaADNom20 + lambdaAQNom20);    // YNN2
          Real mDSNom; // YXADS
          Real mQSNom; // YXAQS
          Real cosNuNom2; // YCODQ2
          Real sinNuNom2; // YSIDQ2
          Real mINom; // YMC
          Real mDNom(start = mD0Nom);            // YMDI / YMDINS
          Real mQNom(start = mQ0Nom);            // YMQI / YMQINS
          ...
          Real xQ; // YXQ  = mQ + lQI
          Real teta0Num; // YNUM
          Real teta0Denom; // YDENOM
          Real uD0(start = - ui0); // YUD
          Real uQ0(start = ur0); // YUQ
          Real lambdaAD02; // YV72
          Real lambdaAQ02; // YV82
          Real E02;     // YNN2
          Real cosNu2; // YCODQ2
          Real sinNu2; // YSIDQ2
          Real mD(start = mD0Pu); // YMDINS
          Real mQ(start = mQ0Pu); // YMQINS
          Real if0(start = 1.0);
          Real DET(start = 1.0);
          Real u1r(start = ur0);
          Real u1i(start = ui0);
          ...
          Real mDV(start = 1.0);
          Real teta0(start = 0.0); // YTETA = ATAN2(YNUM,YDENOM)
          Real tetaInterne0(start = 0.0);
          Real iD0(start = -ii0_0); // YID
          Real iQ0(start = ir0_0);  // YIQ
          ...
          Real lambdaAD0(start = lambdaAD0_0); // YLAMAD
          Real lambdaAQ0(start = lambdaAQ0_0);   // YLAMAQ
          parameter Real E0_0 = (lambdaAD0_0*lambdaAD0_0 + lambdaAQ0_0*lambdaAQ0_0)^(1/2);
          Real E0(start = E0_0);

          Real mDS0(start = 1.0); // YXADS
          Real mQS0(start = 1.0); // YXAQS
          Real mI0(start = 1.0); // YMC
          Real lambdaQ10(start = 1.0);
          Real lambdaQ20(start = 1.0);
          Real lambdaD0(start = 1.0);   // YLAMD
          Real lambdaF0(start = 1.0);   // YLAMF
          Real cm0(start = 1.0);
          Real efd0(start = 1.0);
          Real lMD0(start = 1.0);
          Real lMQ0(start = 1.0);
...
equation
          ...
          u2 = ur0*ur0 + ui0*ui0; // YN = YE*YE + YF*YF
          ONE*ir0 = (p0*ur0 + q0*ui0) / u2; // YIR
          ONE*ii0 = (p0*ui0 - q0*ur0) / u2; // YII

          ONE*rrTfo = ri;          // YRI = RTFO
          ONE*rTfo  = rTfoIn * rs; // RESTFO(INB3) = RESTFO(INB3) * YS
          ONE*xTfo  = xTfoIn * rs; // REATFO(INB3) = REATFO(INB3) * YS

          ONE*mDSat = mDSatIn / ri^nDSat; // RMD
          ONE*mQSat = mQSatIn / ri^nQSat; // RMQ

          // calcul de WLMDV (FMUDMA) (pucalif.f)
          // ------------------------------------

          //{
          xQNom = mQNom + lQINom; // YXQ
          tetaNomNum = uiNom + omegaNom*(xTfoNom+xQNom)*irNom + (rTfoNom+rStatNom)*iiNom; // YNUM
          tetaNomDenom = urNom + (rTfoNom+rStatNom)*irNom - omegaNom*(xTfoNom+xQNom)*iiNom; // YDENOM
          ...
          iDNom = sin(tetaNom)*irNom - cos(tetaNom)*iiNom; // YID
          iQNom = cos(tetaNom)*irNom + sin(tetaNom)*iiNom; // YIQ
          uDNom = sin(tetaNom)*urNom - cos(tetaNom)*uiNom; // YUD
          uQNom = cos(tetaNom)*urNom + sin(tetaNom)*uiNom; // YUQ

          lambdaADNom = -(uQNom + (rStatNom+rTfoNom)*iQNom + omegaNom*(lStatNom+xTfoNom)*iDNom)/omegaNom; // YLAMAD
          lambdaAQNom =  (uDNom + (rStatNom+rTfoNom)*iDNom - omegaNom*(lQINom+xTfoNom)*iQNom)/omegaNom; // YLAMAQ

          lambdaADNom2 = lambdaADNom*lambdaADNom; // YV72
          lambdaAQNom2 = lambdaAQNom*lambdaAQNom; // YV82
          ENom2 = lambdaADNom2 + lambdaAQNom2;    // YNN2

          mDSNom = mD0Nom / (1. + mDSat*ENom2^(nDSat/2.)); // YXADS
          mQSNom = mQ0Nom / (1. + mQSat*ENom2^(nQSat/2.)); // YXAQS

          cosNuNom2 = lambdaADNom2/ENom2; // YCODQ2
          sinNuNom2 = lambdaAQNom2/ENom2; // YSIDQ2

          mINom = mDSNom * cosNuNom2 + mQSNom * sinNuNom2; // YMC

          mDNom = mINom + mSalNom * sinNuNom2;
          mQNom = mINom - mSalNom * cosNuNom2;
          //}
          ifNom = lambdaADNom/mDNom - iDNom; // YRIF
          ...
          // -----------------------------------
          ONE* yScale = if
                          ( rTfoIn > 0. or xTfoIn > 0.)  then SNREF/SN*rrTfo*rrTfo else SNREF/SN;       // YSCALE

          rStat = rStatIn * yScale; // YI = RESARM * yScale;
          lStat = lStatIn * yScale; // YLAMDI = READST * yScale
          lD = lDPu * yScale;       // YLDI = READDI * YSCALE
          lQI = lStatIn * yScale;   // YLQI = REAQST avec REAQST=READST (puing1.f)
          lRot = lRotIn * yScale;   // YLFI = lRotIn * yScale;
          mQ0 = mQ0Pu*yScale;        // YMQI = FMUQST(INB2) * YSCALE (YMQINS)
          lQ1 = lQ1Pu * yScale;     // YLQ1I
          lQ2 = lQ2Pu * yScale;     // YLQ2I
          mD0 = mD0Pu*yScale;        // YMDI = FMUDST(INB2) * YSCALE (YMDINS)
          ONE*mDV  = mDVPu * yScale;     // YMDIM
          mCan = mCanPu * yScale;   // YMRCI = mCan * yScale;

          // calcul des valeurs initiales des variables machine (park2.f)
          // ------------------------------------------------------------
          mSal = mD0 - mQ0;
          //{
          xQ = mQ + lQI; // YXQ  = mQ + lQI

          teta0Num = ui0 + omega_0*(xTfo+xQ)*ir0 + (rTfo+rStat)*ii0; // YNUM
          teta0Denom = ur0 + (rTfo+rStat)*ir0 - omega_0*(xTfo+xQ)*ii0; // YDENOM
          ...
          ONE*iD0 = sin(teta0)*ir0 - cos(teta0)*ii0; // YID
          ONE* iQ0 = cos(teta0)*ir0 + sin(teta0)*ii0; // YIQ
          uD0 = sin(teta0)*ur0 - cos(teta0)*ui0; // YUD
          uQ0 = cos(teta0)*ur0 + sin(teta0)*ui0; // YUQ

          // expression angle moitié
          //ONE*tetaInterne0 = atan2(uD0,uQ0);
          ONE*tetaInterne0 = 2*atan(uD0/(sqrt(uD0*uD0 + uQ0*uQ0)+uQ0));

          ONE*lambdaAD0 = -(uQ0 + (rStat+rTfo)*iQ0 + (lStat+xTfo)*iD0) / omega_0; // YLAMAD
          ONE*lambdaAQ0 =  (uD0 + (rStat+rTfo)*iD0 - (lQI+xTfo)*iQ0) / omega_0;   // YLAMAQ

          lambdaAD02 = lambdaAD0*lambdaAD0; // YV72
          lambdaAQ02 = lambdaAQ0*lambdaAQ0; // YV82
          E02 = lambdaAD02 + lambdaAQ02;    // YNN2
          ONE*E0= sqrt(E02);

          ONE*mDS0 = mD0 / ( 1.+ mDSat*E02^(nDSat/2));  // YXADS
          ONE*mQS0 = mQ0 / ( 1.+ mQSat*E02^(nQSat/2));  // YXAQS

          cosNu2 = lambdaAD02 / E02; // YCODQ2
          sinNu2 = lambdaAQ02 / E02; // YSIDQ2

          ONE* mI0 = mDS0 * cosNu2 + mQS0 * sinNu2; // YMC

          mD = mI0 + mSal * sinNu2;
          mQ = mI0 - mSal * cosNu2;
          //}

          if0 = lambdaAD0/mD - iD0; // YRIF

          ONE*lambdaQ10 = lambdaAQ0;
          ONE*lambdaQ20 = lambdaAQ0;
          ONE*lambdaD0  = lambdaAD0 + mCan*if0; // YLAMD
          ONE*lambdaF0  = lambdaAD0 + (lRot+mCan)*if0; // YLAMF

          ONE*cm0 = (-lambdaAD0 * iQ0 + lambdaAQ0 * iD0) * SNREF / PN; // YCM

          ONE*efd0 = - mDV * if0 / rrTfo; // YEFD = - YMDIM * YRIF / YRI
