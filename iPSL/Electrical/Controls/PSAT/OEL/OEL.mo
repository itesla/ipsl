...
model OEL "PSAT Over Excitation Limiter"
  ...
  Modelica.Blocks.Interfaces.RealInput v "Generator terminal voltage (pu)"
                                                               annotation(Placement(transformation(extent={{-110,68},
            {-90,88}}),                                                                                                    iconTransformation(extent = {{-90, 66}, {-62, 98}})));
  Modelica.Blocks.Interfaces.RealInput p "Active power (pu)"
                                                  annotation(Placement(transformation(extent={{-110,36},
            {-90,56}}),                                                                                                 iconTransformation(extent = {{-90, 32}, {-62, 62}})));
  Modelica.Blocks.Interfaces.RealInput q "Reactive power (pu)"
                                                   annotation(Placement(transformation(extent={{-110,4},
            {-90,24}}),                                                                                                 iconTransformation(extent = {{-90, -6}, {-62, 24}})));
  FieldCurrent field_current(xd = xd, xq = xq) annotation(Placement(transformation(extent={{-82,38},
            {-46,72}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput v_OXL annotation (Placement(
        transformation(extent={{98,28},{118,48}}), iconTransformation(extent={{66,
            6},{94,38}})));
...
equation
  connect(v, field_current.v) annotation(Line(points={{-100,78},{-86,78},{-86,
          63.33},{-75.88,63.33}},                                                                             color = {0, 0, 127}, smooth = Smooth.None));
  connect(p, field_current.p) annotation(Line(points={{-100,46},{-84,46},{-84,
          56.19},{-75.7,56.19}},                                                                             color = {0, 0, 127}, smooth = Smooth.None));
  connect(q, field_current.q) annotation(Line(points={{-100,14},{-84,14},{-84,
          48.71},{-75.7,48.71}},                                                                             color = {0, 0, 127}, smooth = Smooth.None));
