...
model GOVER1 "Steam turbine governor. Developed by AIA. 2013"
  ...
  Modelica.Blocks.Continuous.LimIntegrator imLimitedIntegrator(outMin = 0, k = 1, outMax = 1.1, y_start = init_Integrator) annotation(Placement(transformation(extent = {{-52, 22}, {-34, 40}})));
  Modelica.Blocks.Sources.Constant imSetPoint1(k = 25) annotation(Placement(transformation(extent = {{-52, -38}, {-30, -16}})));
  iPSL.NonElectrical.Continuous.LeadLag imLeadLag(K = 1, T2 = 10, T1 = 3, y_start = init_LeadLag) annotation(Placement(transformation(extent = {{40, 2}, {58, 20}})));
  ...
  Modelica.Blocks.Math.Add3 add3_1(k1 = -25) annotation(Placement(transformation(extent = {{-10, 0}, {10, 20}})));
equation
  connect(imSetPoint.y, imLimitedIntegrator.u) annotation(Line(points = {{-73.1, 31}, {-63.55, 31}, {-63.55, 31}, {-53.8, 31}}, color = {0, 0, 127}));
  connect(imLimitedIntegrator.y, add3_1.u1) annotation(Line(points = {{-33.1, 31}, {-22, 31}, {-22, 18}, {-12, 18}}, color = {0, 0, 127}));
  connect(add3_1.u2, pin_OMEGA) annotation(Line(points = {{-12, 10}, {-40, 10}, {-71, 10}}, color = {0, 0, 127}));
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}})), Icon(graphics={  Rectangle(extent=  {{-60, 60}, {80, -40}}, lineColor=  {0, 0, 255}), Text(extent=  {{-40, -2}, {56, 24}}, lineColor=  {0, 0, 255}, textString=  "GOVER1")}), Documentation(info = "<html>
