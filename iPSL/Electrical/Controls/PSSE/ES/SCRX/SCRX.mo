...
model SCRX "Bus Fed or Solid Fed Static Exciter"
  ...
  parameter Real V_0 "Magnitude of the generator terminal voltage"
    annotation (Dialog(group="Power flow data"));
  ...
  Modelica.Blocks.Interfaces.RealInput ETERM "Ecomp output" annotation (
      Placement(transformation(extent={{-114,0},{-104,10}}), iconTransformation(
          extent={{-126,-72},{-116,-60}})));
  Modelica.Blocks.Interfaces.RealOutput EFD annotation (Placement(
        transformation(extent={{116,-26},{132,-12}}), iconTransformation(extent
          ={{116,-26},{132,-12}})));
  Modelica.Blocks.Math.Add3 V_erro(
    k3=1,
    k1=1,
    k2=-1) annotation (Placement(transformation(extent={{-52,-8},{-36,8}})));
  Modelica.Blocks.Interfaces.RealInput VOTHSG(start=0) "PSS output Upss"
    annotation (Placement(transformation(extent={{-114,-14},{-104,-4}}),
        iconTransformation(extent={{-124,34},{-114,44}})));
  Modelica.Blocks.Interfaces.RealInput VOEL "OEL output" annotation (Placement(
        transformation(extent={{-114,-24},{-104,-14}}), iconTransformation(
          extent={{-124,10},{-114,20}})));
  Modelica.Blocks.Interfaces.RealInput VUEL "UEL output" annotation (Placement(
        transformation(extent={{-114,-34},{-104,-24}}), iconTransformation(
          extent={{-124,-16},{-114,-6}})));
  Modelica.Blocks.Interfaces.RealInput EFD0 annotation (Placement(
        transformation(extent={{-114,10},{-104,20}}), iconTransformation(extent
          ={{-124,-108},{-114,-94}})));
  ...
  Modelica.Blocks.Interfaces.RealInput XADIFD annotation (Placement(
        transformation(extent={{-114,-48},{-104,-36}}), iconTransformation(
          extent={{-126,-38},{-116,-28}})));
  Modelica.Blocks.Interfaces.RealInput ECOMP
    "Ternimal voltage of generator bus" annotation (Placement(transformation(
          extent={{-114,22},{-104,32}}), iconTransformation(extent={{-124,62},{
            -114,72}})));
  ...
  Modelica.Blocks.Logical.Switch switch1
    annotation (Placement(transformation(extent={{72,20},{84,32}})));
...
equation
  ...
  connect(negCurLogic.Efd, EFD) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(const.y, V_erro.u1) annotation (Line(points={{-79,50},{-58,50},{-58,
  ...
  connect(booleanConstant.y, switch1.u2) annotation (Line(points={{52.6,52},{66,
          52},{66,26},{70.8,26}}, color={255,0,255}));
  connect(product.u2, simpleLagLim.y) annotation (Line(points={{40.8,16.4},{36,
          16.4},{36,0},{31,0}}, color={0,0,127}));
  connect(ECOMP, product.u1) annotation (Line(points={{-109,27},{40.8,27},{40.8,
          23.6}}, color={0,0,127}));
  connect(product.y, switch1.u3) annotation (Line(points={{54.6,20},{70.8,20},{
          70.8,21.2}}, color={0,0,127}));
  connect(switch1.u1, simpleLagLim.y) annotation (Line(points={{70.8,30.8},{62,
          30.8},{62,0},{31,0}}, color={0,0,127}));
  connect(switch1.y, negCurLogic.Vd) annotation (Line(points={{84.6,26},{90,26},
          {90,12},{68,12},{68,11},{62.3333,11}}, color={0,0,127}));
  annotation (
    ...
    Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-120},{
          ...
          lineColor={0,0,255},
          textString="Efd")}),
    Icon(coordinateSystem(preserveAspectRatio=true, extent={{-120,-120},{120,80}}),
        graphics={Rectangle(extent={{-120,80},{120,-120}}, lineColor={0,0,255}),
          Text(
          extent={{-114,-48},{-70,-66}},
          lineColor={0,0,255},
