...
model EXST1 "IEEE Type AC2A Excitation System"
  ...
  Modelica.Blocks.Interfaces.RealInput ECOMP annotation (Placement(
        transformation(extent={{-136,48},{-98,86}}), iconTransformation(extent=
            {{-132,52},{-98,86}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput EFD annotation (Placement(
        transformation(extent={{280,-50},{300,-30}}), iconTransformation(extent
          ={{280,-50},{300,-30}})));
  Modelica.Blocks.Interfaces.RealInput EFD0 annotation (Placement(
        ...
        origin={-39,-165},
        rotation=90)));
  Modelica.Blocks.Interfaces.RealInput ETERM annotation (Placement(
        transformation(
        extent={{-19,-19},{19,19}},
        rotation=0,
  ...
  Modelica.Blocks.Interfaces.RealInput VOTHSG annotation (Placement(
        transformation(extent={{-138,-52},{-98,-12}}), iconTransformation(
          extent={{-132,-68},{-98,-34}})));
  Modelica.Blocks.Interfaces.RealInput VOEL
    "Input from voltage over excitation limiter" annotation (Placement(
        transformation(extent={{-138,-110},{-98,-70}}), iconTransformation(
          extent={{-132,-104},{-98,-70}})));
  Modelica.Blocks.Interfaces.RealInput VUEL
    "Input from voltage under excitation limiter" annotation (Placement(
        transformation(extent={{-140,-158},{-100,-118}}), iconTransformation(
          extent={{-132,-142},{-98,-108}})));
...
protected
  Modelica.Blocks.Interfaces.RealOutput EFD1 annotation (Placement(
        transformation(extent={{230,-30},{250,-10}}), iconTransformation(extent
          ={{302,-64},{322,-44}})));
initial equation
  Vm0 = ECOMP;
  Efd0 = EFD0;
...
equation
  ...
  connect(V_REF.y, VERR.u2) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(Vm.y, VERR.u1) annotation (Line(points={{19,-24},{26.8,-24},{26.8,-23.6}},
        color={0,0,127}));
  connect(ECOMP, Vm.u) annotation (Line(points={{-117,67},{-40,67},{-40,-24},{-4,
          -24}}, color={0,0,127}));
  ...
  connect(VOEL, add3_2.u2) annotation (Line(points={{-118,-90},{-90,-90},{-62,-90}},
        color={0,0,127}));
  ...
  connect(K_a.y, Vm1.u)
    annotation (Line(points={{158.4,-20},{172.8,-20}}, color={0,0,127}));
  connect(Vm1.y, EFD1)
    annotation (Line(points={{209.6,-20},{240,-20}}, color={0,0,127}));
  connect(imDerivativeLag.u, EFD1) annotation (Line(points={{144,-80},{220,-80},
          {220,-20},{240,-20}}, color={0,0,127}));
  annotation (
    Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-200},{
          ...
          lineColor={255,0,0},
          textString="VERR")}),
    Icon(coordinateSystem(extent={{-120,-200},{300,100}}, preserveAspectRatio=
            true), graphics={Text(
          extent={{28,-18},{188,-60}},
          lineColor={0,0,255},
          textString="EXST1"),Rectangle(extent={{-100,82},{280,-160}},
