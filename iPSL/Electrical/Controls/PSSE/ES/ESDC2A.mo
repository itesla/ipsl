...
model ESDC2A
  ...
  NonElectrical.Continuous.SimpleLagLimVar simpleLagLimVar(K = K_A, T = T_A, y_start = V_R0) annotation(Placement(transformation(extent = {{80, -10}, {100, 10}})));
  Modelica.Blocks.Interfaces.RealInput VT annotation(Placement(transformation(extent = {{-20, -20}, {20, 20}}, rotation = 90, origin = {102, -120}), iconTransformation(extent = {{-10, -10}, {10, 10}}, rotation = 0, origin = {-200, -70})));
  Modelica.Blocks.Math.Gain gain(k = V_RMIN0) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 90, origin = {82, -70})));
  Modelica.Blocks.Math.Gain gain1(k = V_RMAX0) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 90, origin = {110, -70})));
protected
...
equation
  ...
  connect(simpleLagLimVar.u, hV_GATE.p) annotation(Line(points = {{78, 0}, {60.625, 0}}, color = {0, 0, 127}));
  connect(simpleLagLimVar.y, rotatingExciterLimited.I_C) annotation(Line(points = {{101, 0}, {118.75, 0}}, color = {0, 0, 127}));
  connect(VT, gain.u) annotation(Line(points = {{102, -120}, {102, -92}, {82, -92}, {82, -82}}, color = {0, 0, 127}));
  connect(gain.y, simpleLagLimVar.outMin) annotation(Line(points = {{82, -59}, {82, -36}, {82, -14}}, color = {0, 0, 127}));
