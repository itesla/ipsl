...
model EXAC2 "IEEE Type AC2 Excitation System"
  ...
  Modelica.Blocks.Interfaces.RealInput ECOMP
    "Voltage regultor compensated voltage (pu)"                                          annotation(Placement(transformation(extent = {{-164, 14}, {-152, 26}}), iconTransformation(extent = {{-164, 14}, {-152, 26}})));
  ...
  Modelica.Blocks.Interfaces.RealInput VOEL
    "Maximum excitation limiter signal (pu)"                                         annotation(Placement(transformation(extent = {{-164, -66}, {-152, -54}}), iconTransformation(extent = {{-164, -66}, {-152, -54}})));
  ...
  Modelica.Blocks.Continuous.Derivative imDerivativeLag(k = K_F, T = T_F, y_start = 0, initType = Modelica.Blocks.Types.Init.InitialOutput) annotation(Placement(transformation(extent = {{7, -6}, {-7, 6}}, rotation = 0, origin = {-59, 0})));
  NonElectrical.Continuous.SimpleLag imSimpleLag(K = 1, T = T_R, y_start = EC0) annotation(Placement(transformation(extent = {{-132, -40}, {-116, -24}})));
  Modelica.Blocks.Interfaces.RealInput VUEL
    "Minimum excitation limiter signal (pu)"                                         annotation(Placement(transformation(extent = {{-164, -46}, {-152, -34}}), iconTransformation(extent = {{-164, -46}, {-152, -34}})));
  NonElectrical.Continuous.LeadLag imLeadLag(K = 1, T1 = T_C, T2 = T_B, y_start = VA0 / K_A) annotation(Placement(transformation(extent = {{-60, -34}, {-48, -22}})));
  Modelica.Blocks.Math.Gain gain(k = K_H) annotation(Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 0, origin = {-32, -12})));
  ...
  NonElectrical.Functions.ImSE imSE(SE1 = S_EE_1, SE2 = S_EE_2, E1 = E_1, E2 = E_2) annotation(Placement(transformation(extent = {{92, 14}, {72, 34}})));
  Modelica.Blocks.Math.Gain gain2(k = K_E) annotation(Placement(transformation(extent = {{86, 0}, {78, 8}})));
  Modelica.Blocks.Sources.Constant Vref1(k = V_LR) annotation(Placement(transformation(extent = {{-9, -9}, {9, 9}}, rotation = 270, origin = {-17, 55})));
  Modelica.Blocks.Interfaces.RealInput EFD0
    "Initial value of generator main field voltage (pu)"                                         annotation(Placement(transformation(extent = {{-164, 54}, {-152, 66}}), iconTransformation(extent = {{-164, 54}, {-152, 66}})));
  ...
  Modelica.Blocks.Math.Add add3(k1 = -1, k2 = +1) annotation(Placement(transformation(extent = {{-6, -6}, {6, 6}}, rotation = -90, origin = {-14, 10})));
  Modelica.Blocks.Math.Gain gain3(k = K_L) annotation(Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 270, origin = {-14, -8})));
  Modelica.Blocks.Math.Add add4(k1 = -1) annotation(Placement(transformation(extent = {{68, -32}, {80, -20}})));
  Modelica.Blocks.Continuous.LimIntegrator limIntegrator(k = 1 / T_E, outMax = Modelica.Constants.inf, outMin = 0, initType = Modelica.Blocks.Types.Init.InitialOutput, y_start = VE0) annotation(Placement(transformation(extent = {{92, -32}, {104, -20}})));
  ...
  Modelica.Blocks.Math.Gain gain5(k = K_D) annotation(Placement(transformation(extent = {{5, -5}, {-5, 5}}, rotation = 90, origin = {43, 29})));
  Modelica.Blocks.Math.Add add6(k1 = +1) annotation(Placement(transformation(extent = {{38, 10}, {26, 22}})));
  NonElectrical.Continuous.SimpleLagLim simpleLagLim(K = K_A, T = T_A, y_start = VA0, outMax = V_AMAX, outMin = V_AMIN) annotation(Placement(transformation(extent = {{-36, -34}, {-24, -22}})));
protected
...
equation
  ...
  connect(add2.y, lV_GATE.n1) annotation(Line(points = {{2.6, -24}, {13, -24}, {13, -26.2}}, color = {0, 0, 127}));
  ...
  connect(gain3.y, lV_GATE.n2) annotation(Line(points = {{-14, -12.4}, {-14, -12.4}, {-14, -40}, {10, -40}, {10, -30.6}, {13, -30.6}}, color = {0, 0, 127}));
  connect(lV_GATE.p, gain1.u) annotation(Line(points = {{25.9, -28.5}, {30.95, -28.5}, {30.95, -29}, {31, -29}}, color = {0, 0, 127}));
  ...
  connect(imSE.VE_IN, product.u2) annotation(Line(points = {{92.5, 23.9}, {130, 23.9}, {130, -4.8}, {138.4, -4.8}}, color = {0, 0, 127}));
  connect(gain2.u, product.u2) annotation(Line(points = {{86.8, 4}, {130, 4}, {130, -4.8}, {138.4, -4.8}}, color = {0, 0, 127}));
  connect(add5.u1, imSE.VE_OUT) annotation(Line(points = {{65.2, 15.6}, {68, 15.6}, {68, 23.9}, {71.5, 23.9}}, color = {0, 0, 127}));
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-160, -80}, {160, 80}}), graphics = {Text(extent=  {{-124, -18}, {-114, -26}}, lineColor=  {0, 0, 255}, textString=  "VREF"), Text(extent=  {{-8, 28}, {2, 22}}, lineColor=  {255, 0, 0}, textString=  "V"), Text(extent=  {{-4, 24}, {4, 22}}, lineColor=  {255, 0, 0}, textString=  "FE"), Text(extent=  {{-170, 60}, {-160, 54}}, lineColor=  {255, 0, 0}, textString=  "V"), Text(extent=  {{-160, 56}, {-166, 54}}, lineColor=  {255, 0, 0}, textString=  "F")}), Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-160, -80}, {160, 80}}), graphics = {Rectangle(extent=  {{-160, 80}, {160, -80}}, lineColor=  {0, 0, 255}), Text(extent=  {{-150, 64}, {-128, 54}}, lineColor=  {0, 0, 255}, textString=  "EFD0"), Text(extent=  {{-150, 28}, {-120, 12}}, lineColor=  {0, 0, 255}, textString=  "ECOMP"), Text(extent=  {{-154, -16}, {-106, -26}}, lineColor=  {0, 0, 255}, textString=  "VOTHSG"), Text(extent=  {{-154, -36}, {-122, -46}}, lineColor=  {0, 0, 255}, textString=  "VUEL"), Text(extent=  {{128, 8}, {158, -8}}, lineColor=  {0, 0, 255}, textString=  "EFD"), Text(extent=  {{-52, 32}, {52, -30}}, lineColor=  {0, 0, 255}, textString=  "EXAC2"), Text(extent=  {{-150, 50}, {-118, 30}}, lineColor=  {0, 0, 255}, textString=  "XADIFD"), Text(extent=  {{-154, -56}, {-122, -66}}, lineColor=  {0, 0, 255}, textString=  "VOEL")}), Documentation(info = "<html>
