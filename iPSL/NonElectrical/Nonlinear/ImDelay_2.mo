...
model ImDelay_2 "Delay_2.
  ...
  iPSL.NonElectrical.Continuous.ImSimpleLag imSimpleLag(
    K=1,
    T=T/20,
    nStartValue=SimpleLagStartValue)
    annotation (Placement(transformation(extent={{-40,-20},{-2,20}})));
  iPSL.NonElectrical.Nonlinear.ImDelay_1 imDelay_1_1(T=T)
    annotation (Placement(transformation(extent={{-4,-18},{36,18}})));
equation
  //n1 = delay(p1, T);
  connect(p1, imSimpleLag.p1) annotation(Line(points = {{-51, 0}, {-26.89, 0}}, color = {0, 0, 127}, smooth = Smooth.None));
