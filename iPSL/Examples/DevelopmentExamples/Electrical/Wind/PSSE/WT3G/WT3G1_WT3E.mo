within iPSL.Examples.DevelopmentExamples.Electrical.Wind.PSSE.WT3G;
...
model WT3G1_WT3E
  ...
  iPSL.Electrical.Branches.PwLine pwLine5(R = Rt2, X = Xt2, G = 0, B = 0) annotation(Placement(transformation(extent = {{38, -6}, {58, 14}})));
  ...
  iPSL.Electrical.Banks.PSSE.CSVGN1 cSVGN1_1(K = 100, T1 = 0, T2 = 0, T5 = 0, VMIN = 0, CBASE = 100, MBASE = 100, SBASE = 100, VMAX = 1.1, v0 = V3, anglev0 = A3, T3 = 100, T4 = 0, Vc0 = V5, Psh = Psh, Qsh = Qsh) annotation(Placement(transformation(extent = {{-52, -60}, {-18, -26}})));
  iPSL.Electrical.Banks.PSSE.Shunt shunt1(B = -0.25, G = 0) annotation(Placement(transformation(extent = {{46, -36}, {66, -16}})));
  iPSL.Electrical.Events.PwFault pwFault(R = 0.5, X = 0.5, t1 = 1, t2 = 1.5) annotation(Placement(transformation(extent = {{32, -34}, {48, -18}})));
  iPSL.Electrical.SystemBase SysData annotation(Placement(transformation(extent = {{-126, 48}, {-102, 68}})));
  iPSL.Electrical.Wind.PSSE.WT3G.WT3G1 wT3G1_pt(eterm = V5, anglev0 = A5, pelec = P5, qelec = Q5, Sbase = 100, Zs(im = Zi), Xeq = dyrw[1, 1], Kpll = dyrw[1, 2], Kipll = dyrw[1, 3], Pllmax = dyrw[1, 4], Prated = dyrw[1, 5], Mbase = 1.5, wbase = 2 * pi * 50) annotation(Placement(transformation(extent = {{-22, 23}, {22, -23}}, rotation = 180, origin = {102, 7})));
  iPSL.Electrical.Wind.PSSE.WT3G.WT3E1 wT3E1_1(VARFLG = 1, v0 = V5, p0 = P5 / 1.5, q0 = Q5 / 1.5, VLRFLG = 1, Vref = 1) annotation(Placement(transformation(extent = {{148, -16}, {214, 30}})));
equation
  ...
  connect(pwLine5.p, pwLine2.n) annotation(Line(points = {{41, 4}, {24, 4}, {24, -8}, {3, -8}}, color = {0, 0, 255}, smooth = Smooth.None));
  connect(shunt1.p, pwLine5.n) annotation(Line(points = {{55.6, -18.4}, {55.6, -7.2}, {55, -7.2}, {55, 4}}, color = {0, 0, 255}, smooth = Smooth.None));
  connect(pwFault.p, pwLine2.n) annotation(Line(points = {{30.6667, -26}, {30.6667, 4}, {24, 4}, {24, -8}, {3, -8}}, color = {0, 0, 255}, smooth = Smooth.None));
  connect(wT3G1_pt.Ipcmd, wT3E1_1.WIPCMD) annotation(Line(points = {{121.8, 13.9}, {132.78, 13.9}, {132.78, 15.2143}, {146.35, 15.2143}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(pwLine5.n, wT3G1_pt.p) annotation(Line(points = {{55, 4}, {70, 4}, {70, 7}, {77.8, 7}}, color = {0, 0, 255}, smooth = Smooth.None));
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-140, -80}, {160, 100}})), Icon(coordinateSystem(extent = {{-140, -80}, {160, 100}})), Documentation(revisions = "<html>
<!--DISCLAIMER-->
