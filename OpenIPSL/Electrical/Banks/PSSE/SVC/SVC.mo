...
model SVC "On bus 10106 & 10114"
  ...
  Modelica.Blocks.Sources.Constant imSetPoint(k=Vref) annotation (
      Placement(transformation(extent={{-76,20},{-64,32}})));
  Modelica.Blocks.Sources.Constant imSetPoint1(k=Bref) annotation (
      Placement(transformation(extent={{-52,20},{-38,34}})));
  ...
  OpenIPSL.NonElectrical.Logical.Relay3 imRelay annotation (Placement(
        transformation(extent={{30,-40},{50,-20}})));
  Modelica.Blocks.Sources.Constant Q_capacitors(k=Mvar_C)
    "If Verr>Vov" annotation (Placement(transformation(extent={{-20,-20},
            {-10,-10}})));
  Modelica.Blocks.Sources.Constant Q_Reactors(k=Mvar_R)
    "If Verr<-Vov" annotation (Placement(transformation(extent={{-20,-38},
            {-10,-28}})));
  Modelica.Blocks.Math.Gain imGain(k=1/Sbase) annotation (Placement(
        transformation(extent={{60,-36},{72,-24}})));
...
equation
  ...
  connect(shunt.p, VIB) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(imRelay.y, imGain.u) annotation (Line(points={{51.5,-30},{
          51.5,-30},{58.8,-30}},
                            color={0,0,127}));
  connect(shunt.Q, imGain.y) annotation (Line(points={{79,-30},{79,
          -30},{72.6,-30}},
                        color={0,0,127}));
  connect(imLimitedSimpleLag.y, imRelay.u2) annotation (Line(points={{82.8,8},
          {86,8},{86,-16},{20,-16},{20,-22.5},{28,-22.5}},
        color={0,0,127}));
  connect(imLimited.y, imLimitedSimpleLag.u) annotation (Line(points={
          {58.8,8},{60,8},{64.4,8}}, color={0,0,127}));
  connect(imLeadLag1.y, imLimited.u) annotation (Line(points={{34.8,8},
          {34.8,8},{40.4,8}}, color={0,0,127}));
  connect(imLeadLag.y, imLeadLag1.u)
    annotation (Line(points={{10.8,8},{16.4,8}}, color={0,0,127}));
  connect(Q_Reactors.y, imRelay.u4) annotation (Line(points={{-9.5,
          -33},{28,-33},{28,-32.5}},
                                color={0,0,127}));
  connect(Q_capacitors.y, imRelay.u3) annotation (Line(points={{-9.5,
          -15},{0,-15},{0,-27.5},{28,-27.5}},
                                           color={0,0,127}));
  connect(add.u2, absoluteVoltage.v) annotation (Line(points={{-59.2,2.4},
          {-62,2.4},{-62,-4.8},{-65.2,-4.8}}, color={0,0,127}));
  connect(imSetPoint.y, add.u1) annotation (Line(points={{-63.4,26},{-59.2,
          26},{-59.2,9.6}}, color={0,0,127}));
  connect(add3_1.y, imLeadLag.u) annotation (Line(points={{-11.4,8},{-10.65,
          8},{-7.6,8}}, color={0,0,127}));
  connect(imSetPoint2.y, add3_1.u3) annotation (Line(points={{-39.4,-14},
          {-34,-14},{-34,3.2},{-25.2,3.2}}, color={0,0,127}));
  connect(imSetPoint1.y, add3_1.u1) annotation (Line(points={{-37.3,27},
          {-34,27},{-34,12.8},{-25.2,12.8}}, color={0,0,127}));
  connect(add.y, add3_1.u2) annotation (Line(points={{-45.4,6},{-25.2,
          6},{-25.2,8}}, color={0,0,127}));
  connect(imRelay.u1, add3_1.u2) annotation (Line(points={{28,-37.5},
          {0,-37.5},{0,-46},{-36,-46},{-36,8},{-25.2,8}},  color={0,0,
          127}));
  annotation (Diagram(coordinateSystem(initialScale=0.1),
        ...
        Text(
          extent={{-80,-14},{-68,-20}},
          lineColor={255,0,0},
          textString="|VB|")}), Icon(coordinateSystem(initialScale=
            0.1),                                   graphics={
        ...
        Line(
          ...
          color={0,0,255},
          smooth=Smooth.Bezier),
        Line(points={{50,-40},{90,-40}},  color={0,0,255}),
        Line(points={{60,-50},{80,-50}}, color={0,0,255}),
        Line(points={{10,0},{70,0},{70,-40}}, color={0,0,255})}));
