...
model Network
  ...
  iPSL.Electrical.Buses.Bus bus6
    annotation (Placement(transformation(extent={{-110,20},{-90,40}})));
  iPSL.Electrical.Buses.Bus bus7
    annotation (Placement(transformation(extent={{-70,20},{-50,40}})));
  iPSL.Electrical.Buses.Bus bus8
    annotation (Placement(transformation(extent={{-10,20},{10,40}})));
  iPSL.Electrical.Buses.Bus bus9
    annotation (Placement(transformation(extent={{50,20},{70,40}})));
  iPSL.Electrical.Buses.Bus bus10
    annotation (Placement(transformation(extent={{90,20},{110,40}})));
  iPSL.Electrical.Buses.Bus bus11
    annotation (Placement(transformation(extent={{130,20},{150,40}})));
  TwoAreas.Groups.G1 g1
    annotation (Placement(transformation(extent={{-214,24},{-202,36}})));
  TwoAreas.Groups.G2 g2
    annotation (Placement(transformation(extent={{-214,-16},{-202,-4}})));
  TwoAreas.Groups.G3 g3 annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=180,
        origin={208,30})));
  TwoAreas.Groups.G4 g4 annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=180,
        origin={208,-10})));
  ...
  inner iPSL.Electrical.SystemBase SysData
    annotation (Placement(transformation(extent={{-220,40},{-196,60}})));
equation
  connect(g1.pwPin, bus1.p) annotation (Line(points={{-201,30},{-190.5,30},{
          -180,30}},
                color={0,0,255}));
  connect(g2.pwPin, bus2.p) annotation (Line(points={{-201,-10},{-190.5,-10},
          {-180,-10}},color={0,0,255}));
  connect(Line6_7.n, bus7.p)
    annotation (Line(points={{-73,30},{-66.5,30},{-60,30}},
                                                        color={0,0,255}));
  connect(Line6_7.p, bus6.p)
    annotation (Line(points={{-87,30},{-93.5,30},{-100,30}},
                                                           color={0,0,255}));
  connect(Line5_6.n, bus6.p)
    annotation (Line(points={{-113,30},{-106.5,30},{-100,30}},
                                                            color={0,0,255}));
  connect(Line5_6.p, bus5.p)
    annotation (Line(points={{-127,30},{-133.5,30},{-140,30}},
                                                            color={0,0,255}));
  connect(Line8_9_2.n, bus9.p) annotation (Line(points={{37,20},{54,20},{54,
          30},{60,30}},
                     color={0,0,255}));
  connect(Line8_9_1.n, bus9.p) annotation (Line(points={{37,40},{54,40},{54,
          30},{60,30}},
                     color={0,0,255}));
  connect(Line8_9_2.p, bus8.p) annotation (Line(points={{23,20},{6,20},{6,30},
          {0,30}},  color={0,0,255}));
  connect(Line8_9_1.p, bus8.p) annotation (Line(points={{23,40},{6,40},{6,30},
          {0,30}},  color={0,0,255}));
  connect(Line7_8_2.n, bus8.p) annotation (Line(points={{-23,20},{-6,20},{-6,
          22},{-6,30},{0,30}},
                            color={0,0,255}));
  connect(Line7_8_1.n, bus8.p) annotation (Line(points={{-23,40},{-6,40},{-6,
          30},{0,30}},
                    color={0,0,255}));
  connect(Line7_8_1.p, bus7.p) annotation (Line(points={{-37,40},{-54,40},{
          -54,30},{-60,30}},
                    color={0,0,255}));
  connect(Line7_8_2.p, bus7.p) annotation (Line(points={{-37,20},{-54,20},{
          -54,30},{-60,30}},
                    color={0,0,255}));
  connect(bus9.p, Line9_10.p)
    annotation (Line(points={{60,30},{73,30}},   color={0,0,255}));
  connect(Line9_10.n, bus10.p)
    annotation (Line(points={{87,30},{93.5,30},{100,30}},   color={0,0,255}));
  connect(bus10.p, Line10_11.p)
    annotation (Line(points={{100,30},{113,30}}, color={0,0,255}));
  connect(Line10_11.n, bus11.p)
    annotation (Line(points={{127,30},{133.5,30},{140,30}}, color={0,0,255}));
  connect(g4.pwPin, bus4.p) annotation (Line(points={{201,-10},{201,-10},{180,
          -10}}, color={0,0,255}));
  connect(g3.pwPin, bus3.p)
    annotation (Line(points={{201,30},{180,30}},          color={0,0,255}));
  connect(Load7.p, bus7.p) annotation (Line(points={{-64,0.6},{-64,0.6},{-64,30},
          {-60,30}},color={0,0,255}));
  connect(Load9.p, Line9_10.p) annotation (Line(points={{66,0.6},{66,0.6},{66,
          30},{73,30}},      color={0,0,255}));
  connect(pwFault.p, bus8.p)
    annotation (Line(points={{2,-4.33333},{2,30},{0,30}},
                                                       color={0,0,255}));
  connect(bus1.p, Line5_1.p)
    annotation (Line(points={{-180,30},{-174,30},{-167,30}},
                                                           color={0,0,255}));
  connect(bus5.p, Line5_1.n)
    annotation (Line(points={{-140,30},{-146,30},{-153,30}},
                                                          color={0,0,255}));
  connect(bus2.p, Line5_2.p) annotation (Line(points={{-180,-10},{-174,-10},{
          -167,-10}},color={0,0,255}));
  connect(Line5_2.n, bus6.p) annotation (Line(points={{-153,-10},{-104,-10},{
          -104,30},{-100,30}},
                             color={0,0,255}));
  connect(bus11.p, Line5_3.p)
    annotation (Line(points={{140,30},{153,30}}, color={0,0,255}));
  connect(Line5_3.n, bus3.p) annotation (Line(points={{167,30},{173.5,30},{
          180,30}}, color={0,0,255}));
  connect(bus4.p, Line5_4.n)
    annotation (Line(points={{180,-10},{167,-10}}, color={0,0,255}));
  connect(Line5_4.p, Line10_11.p) annotation (Line(points={{153,-10},{104,-10},
          {104,30},{113,30}}, color={0,0,255}));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-220,
            -40},{220,60}})), Icon(coordinateSystem(extent={{-220,-40},{220,
            60}})),
