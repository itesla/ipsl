...
model Gen_gov_exc_stab
  ...
  OpenIPSL.Electrical.Controls.PSSE.PSS.IEEEST iEEEST(
    ...
    L_SMIN=L_SMIN,
    V_CU=V_CU,
    V_CL=V_CL) annotation (Placement(transformation(extent={{90,-49},{18,-21}})));
  Modelica.Blocks.Sources.Constant const(k=0) annotation (Placement(transformation(extent={{0,-58},{-12,-46}})));
  Modelica.Blocks.Sources.Constant const1(k=0) annotation (Placement(transformation(extent={{74,-14},{86,-2}})));
equation
  connect(gENSAL.p, pwPin) annotation (Line(points={{28.2,26},{64,26},{90,26}}, color={0,0,255}));
  connect(gENSAL.PMECH0, hYGOV.PMECH0) annotation (Line(points={{27.56,16.4},{40,16.4},{40,77.5},{25.3,77.5}}, color={0,0,127}));
