...
model FourthOrder_AVRII_Discrete "5-bus system with 4th-order machine and AVRTypeII_Discrete"
  ...
  Modelica.Blocks.Sources.Constant vref2(k=1.198705028536746) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-199,27})));
  Modelica_Synchronous.RealSignals.Sampler.Hold hold1(y_start=1.046883400898693) annotation (Placement(transformation(extent={{-136,0},{-124,12}})));
  Modelica_Synchronous.RealSignals.Sampler.Sample sample1 annotation (Placement(transformation(extent={{-184,22},{-172,34}})));
  Modelica_Synchronous.RealSignals.Sampler.SampleClocked sample2 annotation (Placement(transformation(extent={{-190,-6},{-178,6}})));
...
equation
  ...
  connect(periodicClock1.y, sample2.clock)
    annotation (Line(
      ...
      thickness=0.5,
      smooth=Smooth.None));
  connect(hold1.y, order4.vf) annotation (Line(points={{-123.4,6},{-112,6},{-112,5}}, color={0,0,127}));
  connect(sample2.u, order4.v) annotation (Line(points={{-191.2,0},{-200,0},{-200,-20},{-88,-20},{-88,3},{-91,3}}, color={0,0,127}));
  annotation (
