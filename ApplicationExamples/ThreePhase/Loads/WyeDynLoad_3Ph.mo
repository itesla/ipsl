...
model WyeDynLoad_3Ph
 ...
 import Modelica.Blocks.Interfaces.*;
  parameter Real Sn=SysData.S_b "Power rating (MVA)" annotation (Dialog(group="Power flow"));
   Modelica.Blocks.Interfaces.RealInput DynFact "Load Curve (pu)"  annotation (Placement(
   visible = true, 
   transformation(origin = {-88, 0}, extent = {{-20, -20}, {20, 20}}, rotation = 0), 
   iconTransformation(origin = {-52, 0}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
 ...
 parameter Integer ModelType = 0
  "0- Constant Power Model, 1- ZIP Model;"
    annotation (choices(
      choice=0 "Constant Power",
      choice=1 "ZIP Model"),Dialog(group="Power flow"));  
...
protected
  Real [1,6] TPhasePower = [P_a/(Sn/3), P_b/(Sn/3), P_c/(Sn/3), Q_a/(Sn/3), Q_b/(Sn/3), Q_c/(Sn/3)]*DynFact;
