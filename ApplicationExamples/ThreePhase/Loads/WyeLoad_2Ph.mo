...
model WyeLoad_2Ph
  ...
  OpenIPSL.Interfaces.PwPin A annotation(Placement(transformation(extent = {{35.0, 0.0}, {55.0, 20.0}}, origin = {0.0, 0.0}, rotation = 0), iconTransformation(extent = {{-80.0, 0.0}, {-60.0, 20.0}}, origin = {159, 100}, rotation = 0), visible = true));
  OpenIPSL.Interfaces.PwPin B annotation(Placement(transformation(extent = {{-35.0, 0.0}, {-55.0, 20.0}}, origin = {0.0, 0.0}, rotation = 0), iconTransformation(extent = {{-80.0, 0.0}, {-60.0, 20.0}}, origin = {70, 100}, rotation = 0), visible = true));
  ...
  parameter Real C_pb = 0 "Percentage of Constant Impedance Load] for Phase B (%)" annotation(
    Dialog(group = "Load Parameters for ZIP Model"));
protected
  parameter Real[1, 4] TPhasePower = [P_a / (Sn / 3), P_b / (Sn / 3), Q_a / (Sn / 3), Q_b / (Sn / 3)];
  parameter Real[1, 6] ZIP_coef = [A_pa / 100, B_pa / 100, C_pa / 100, A_pb / 100, B_pb / 100, C_pb / 100];
  ...
  function PowerDefinition
    input Integer ModelType;
    input Real[1, 4] TPhasePower;
    input Real[1, 2] Coef;
    output Real[1, 4] Cor_Power;
  protected
    ...
    parameter Real Qan = TPhasePower[1, 3];
    parameter Real Qbn = TPhasePower[1, 4];
  ...
  algorithm
    if ModelType == 0 then
      Cor_Power := [Pan, Pbn, Qan, Qbn];
    elseif ModelType == 1 then
      Cor_Power := [Pa_cor, Pb_cor, Qa_cor, Qb_cor];
    end if;
  end PowerDefinition;

  Real[1, 4] Power = PowerDefinition(ModelType, TPhasePower, Coef);
  Real Pa = Power[1, 1];
  Real Pb = Power[1, 2];
  Real Qa = Power[1, 3];
  Real Qb = Power[1, 4];
