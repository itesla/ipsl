...
model Bus_2Ph
      ...
      OpenIPSL.Interfaces.PwPin p1(vr(start=V_1*cos(angle_1*Modelica.Constants.pi/180)), vi(start=V_1*sin(angle_1*Modelica.Constants.pi/180)))
        annotation (Placement(transformation(extent={{-10,35},{10,55}})));
      OpenIPSL.Interfaces.PwPin p2(vr(start=V_2*cos(angle_2*Modelica.Constants.pi/180)), vi(start=V_2*sin(angle_2*Modelica.Constants.pi/180)))
      ...
      parameter Real V_2=1 "Voltage magnitude for phase 2 (pu)" annotation (Dialog(group="Power flow data"));
