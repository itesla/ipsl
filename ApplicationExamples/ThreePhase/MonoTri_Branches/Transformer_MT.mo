...
model Transformer_MT "Transformer modeled in a PI element with a hybrid interface Single Phase x Three Phase"

  import ThreePhase.MonoTri_Branches.TransformerFcn.Yg_Yg;
  import ThreePhase.MonoTri_Branches.TransformerFcn.D_D;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Y_Y;
  import ThreePhase.MonoTri_Branches.TransformerFcn.D_Yg;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Yg_D;
  import ThreePhase.MonoTri_Branches.TransformerFcn.D_Y;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Y_D;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Y_Yg;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Yg_Y;
  
  import ThreePhase.MonoTri_Branches.TransformerFcn.Yg_Yg_FinImp;
  import ThreePhase.MonoTri_Branches.TransformerFcn.D_D_FinImp;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Y_Y_FinImp;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Y_Yg_FinImp;
  import ThreePhase.MonoTri_Branches.TransformerFcn.Yg_Y_FinImp;
  ...
  "0- Assuming that the negative and zero norton admittances are infinite (Aproximation), 1- Considering that the negative and zero norton admittances finite values;"
    annotation (Dialog(group="Selection of model"),choices(
      ...
      choice=1 "Norton admittances have finite values (Only possible if the transformer connection does not introduce a phase displacement)"));
  ...
  parameter Real X=0.001 "Reactance (pu system base)" annotation (Dialog(group="Transformer parameters"));
  parameter Real R=0.1 "Resistance (pu system base)" annotation (Dialog(group="Transformer parameters"));
  
  parameter Real G_0 = 0 "Zero sequence conductance in p.u." annotation (Dialog(group="Norton equivalent admittances in terminal K - Considering that the negative and zero norton admittances have finite values"));                                   
  parameter Real B_0 = 0 "Zero sequence susceptance in p.u." annotation (Dialog(group="Norton equivalent admittances in terminal K - Considering that the negative and zero norton admittances have finite values"));                                   
  parameter Real G_1 = 0 "Positive sequence conductance in p.u." annotation (Dialog(group="Norton equivalent admittances in terminal K - Considering that the negative and zero norton admittances have finite values"));                                   
  parameter Real B_1 = 0 "Positive sequence susceptance in p.u." annotation (Dialog(group="Norton equivalent admittances in terminal K - Considering that the negative and zero norton admittances have finite values"));                                   
  parameter Real G_2 = 0 "Negative sequence conductance in p.u." annotation (Dialog(group="Norton equivalent admittances in terminal K - Considering that the negative and zero norton admittances have finite values"));                                   
  parameter Real B_2 = 0 "Negative sequence susceptance in p.u." annotation (Dialog(group="Norton equivalent admittances in terminal K - Considering that the negative and zero norton admittances have finite values")); 
...
protected
  ...
  parameter Real [1,18] Y012 = [G_0, B_0, 0, 0, 0, 0, 0, 0, G_1, B_1, 0, 0, 0, 0, 0, 0, G_2, B_2];
   ...
   parameter Real[1,32] Aux = ConnectionType(Connection, ModelType, X, R, tap, Y012);
...
equation
 // Equations related to the MonoxTri interface
  ...
  C.ii = C3r*p.vi+C3i*p.vr + D31r*A.vi+D31i*A.vr + D32r*B.vi+D32i*B.vr + D33r*C.vi+D33i*C.vr;
    ...
    Icon(graphics = {Ellipse(lineColor = {0, 0, 255}, extent = {{-46, 30}, {8, -30}}, endAngle = 360), Ellipse(lineColor = {0, 0, 255}, extent = {{-10, 30}, {44, -30}}, endAngle = 360), 
    Line(points = {{100, 0}, {44, 0}, {44, 0}}, color = {0, 0, 255}), 
    Line(points = {{-100, 0}, {-46, 0}}, color = {0, 0, 255}), 
    Line(points = {{110, 90}, {37, 20}}, color = {0, 0, 255}),  
    Line(points = {{110, -90}, {37, -20}}, color = {0, 0, 255}), 
    Text(lineColor = {28, 108, 200}, extent = {{-38, 20}, {-4, -20}}, textString = "K"), 
    Text(lineColor = {28, 108, 200}, extent = {{4, 18}, {32, -16}}, textString = "M"), 
    Text(origin = {-56, 64}, lineColor = {28, 108, 200}, extent = {{4, 18}, {104, -34}}, textString = "Hybrid Transformer")}, coordinateSystem(initialScale = 0.1)),
